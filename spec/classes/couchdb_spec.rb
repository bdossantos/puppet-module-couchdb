...
describe 'couchdb', :type => :class do
  ...
  it do
    should contain_exec('download').with({
      'command' => '/usr/bin/wget -q http://mir2.ovh.net/ftp.apache.org/dist/couchdb/1.2.1/apache-couchdb-1.2.1.tar.gz -O apache-couchdb-1.2.1.tar.gz',
      'cwd'     => '/usr/local/src',
    })
  end
  ...
  it do
    should contain_exec('extract').with({
      'command' => '/bin/tar -xzvf apache-couchdb-1.2.1.tar.gz',
      'cwd'     => '/usr/local/src',
    })
  end
  ...
  it do
    should contain_exec('configure').with({
      'cwd' => '/usr/local/src/apache-couchdb-1.2.1',
    })
  end
